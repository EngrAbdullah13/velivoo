...
import { DELIVERY_QUEUE_MAX_ATTEMPTS, DELIVERY_QUEUE_RETRY_DELAY_MS } from "../../../packages/domain/src/phase2/delivery-pacing.js";
...
const runtime=createPhase2Worker(redisUrl,async(job:Phase2Job,queue)=>{
  ...
  else if(job.type==="phase2.message.render"){await service.renderMessage(job.workspaceId,job.messageId);await queue.add("phase2.message.submit",{type:"phase2.message.submit",workspaceId:job.workspaceId,messageId:job.messageId},{jobId:`phase2.message.submit:${job.workspaceId}:${job.messageId}`,attempts:DELIVERY_QUEUE_MAX_ATTEMPTS,backoff:{type:"fixed",delay:DELIVERY_QUEUE_RETRY_DELAY_MS},removeOnComplete:1000,removeOnFail:1000});}
...
runtime.worker.on("completed",j=>console.log(JSON.stringify({event:"phase2.job.completed",id:j.id,name:j.name})));runtime.worker.on("failed",(j,e)=>{if(e.message!=="DELIVERY_CAPACITY_DEFERRED")console.error(JSON.stringify({event:"phase2.job.failed",id:j?.id,name:j?.name,error:e.message}))});
