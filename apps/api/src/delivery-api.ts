...
function headers(){return {"content-type":"application/json; charset=utf-8","access-control-allow-origin":origin,"access-control-allow-credentials":"true","access-control-allow-headers":"authorization,content-type,x-dev-user,x-request-id","access-control-allow-methods":"GET,POST,PATCH,OPTIONS","cache-control":"no-store","x-content-type-options":"nosniff"}}
...
async function json(req:IncomingMessage){const chunks:Buffer[]=[];let size=0;for await(const c of req){const b=Buffer.from(c);size+=b.length;if(size>2_000_000)throw new Error("REQUEST_TOO_LARGE");chunks.push(b)}return chunks.length?JSON.parse(Buffer.concat(chunks).toString("utf8")):{} }
...
function code(e:unknown){const s=e instanceof Error?e.message:"ERROR";if(s.startsWith("AUTH_"))return 401;if(s.startsWith("FORBIDDEN")||s==="WORKSPACE_ACCESS_DENIED")return 403;if(s.includes("NOT_FOUND"))return 404;if(s.includes("CONFLICT"))return 409;return 400}
