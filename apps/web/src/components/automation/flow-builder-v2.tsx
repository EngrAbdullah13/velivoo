...
import type { Connection, Graph, Step, Issue, FlowDetail, BuilderOptions, RuleOptions, Profile, ActivationReview as Review, Simulation } from './flow-types';
import { profileName } from './flow-types';
import { FlowCanvas, stepLabels, stepSummary, triggerLabel } from './flow-canvas';
import { FlowRules, RuleEditor, eligibleRule } from './flow-rule-editor';
...
export function FlowBuilderV2({ workspaceId, flowId }: { workspaceId: string; flowId: string }) {
  ...
  const change = (next: Graph) => {
    ...
    setHistory(items => [...items.slice(-49), clone(graph)]); setFuture([]); setGraph(next); setSaveState('unsaved'); setSimulation(null);
  };
  ...
  const persist = useCallback(async (override?: Graph, overrideName?: string) => {
    ...
    const request = phase3Api<FlowDetail>(`/api/v1/workspaces/${workspaceId}/flows/${flowId}`, { method: 'PATCH', body: JSON.stringify({ graph: snapshot, name: snapshotName, rowVersion: currentFlow.rowVersion }) })
      .then(saved => {
        const merged = flowRef.current ? { ...flowRef.current, ...saved, versions: flowRef.current.versions } : saved;
        const graphMatches = json(graphRef.current) === json(snapshot);
        const nameMatches = nameRef.current.trim() === snapshotName;
        flowRef.current = merged;
        setFlow(merged);
  ...
  return <div className={s.builderShell}>
    <header className={s.builderHeader}>
      ...
      <div className={s.headerActions}><Status value={flow.status}/><button className={s.iconButton} disabled={!history.length || !canWrite} onClick={undo} title="Undo"><FlowIcon name="undo"/></button><button className={s.iconButton} disabled={!future.length || !canWrite} onClick={redo} title="Redo"><FlowIcon name="redo"/></button><button className={s.secondary} disabled={busy} onClick={() => void simulate()}>Preview path</button><button className={s.secondary} disabled={busy} onClick={() => void validateDraft()}>Review</button><button className={s.primary} disabled={busy || !flow.capabilities.canActivate} onClick={() => void changeStatus()}>{flow.status === 'active' || flow.status === 'testing' ? <><FlowIcon name="pause"/>Pause</> : flow.status === 'paused' ? <><FlowIcon name="play"/>Resume</> : 'Review & turn on'}</button></div>
    </header>
    <div className={s.builderBody}>
      ...
      <main className={s.canvasArea}><div className={s.canvasToolbar}><div><strong>{graph.nodes.length - graph.nodes.filter(node => node.type === 'end').length} steps</strong><span>{blocking ? `${blocking} blocking issue${blocking === 1 ? '' : 's'}` : 'Ready for review'}</span></div><div><button className={s.iconButton} onClick={() => setZoom(value => Math.max(50, value - 10))}>−</button><span>{zoom}%</span><button className={s.iconButton} onClick={() => setZoom(value => Math.min(130, value + 10))}>+</button><button className={s.secondary} onClick={() => setZoom(90)}>Reset</button></div></div><FlowCanvas graph={graph} options={options} issues={issues} selected={selected} onSelect={id => { setSelected(id); setPanel('settings'); }} onInsert={edge => setInsertTarget(edge)} zoom={zoom} readOnly={!canWrite}/>{simulation && <SimulationTray simulation={simulation} options={options} graph={graph} close={() => setSimulation(null)}/>}</main>
    </div>
    ...
    {insertTarget && <StepPicker close={() => setInsertTarget(null)} choose={chooseStep}/>} {review && <ActivationDialog review={review} graph={graph} profiles={profiles} query={profileQuery} profileId={profileId} busy={busy} setQuery={setProfileQuery} setProfileId={setProfileId} close={() => setReview(null)} activate={activate}/>} 
  </div>;
}
...
function TriggerSettings({ graph, options, edit }: { graph: Graph; options: BuilderOptions; edit: (fn: (graph: Graph) => Graph) => void }) {
  const setType = (type: string) => edit(current => ({ ...current, trigger: type === 'list_joined' ? { type, listId: '', enrollmentMode: 'future_only' } : type === 'segment_entered' ? { type, segmentId: '' } : type === 'generic_event' ? { type, eventName: '', schemaVersion: 1 } : type === 'profile_date' ? { type, field: '', hour: 9, minute: 0, timezonePolicy: 'profile_then_workspace' } : { type: 'manual_test' }, entryPolicy: { mode: type === 'manual_test' ? 'once' : 'once_per_event' } }));
...
function NodeSettings({ workspaceId, node, options, ruleOptions, replace }: { workspaceId: string; node: Step; options: BuilderOptions; ruleOptions: RuleOptions; replace: (node: Step) => void }) {
  ...
  if (node.type === 'conditional') return <div className={s.formStack}><div className={s.contract}><strong>Evaluate when the contact reaches this step</strong><span>Choose profile data, list or segment membership, consent, suppression, email engagement, customer events, or nested AND/OR groups.</span></div><RuleEditor rule={node.rule ?? eligibleRule} options={ruleOptions} onChange={rule => replace({ ...node, rule })}/><div className={s.branchLegend}><span><i data-branch="yes"/>Yes — condition matched</span><span><i data-branch="no"/>No — condition did not match</span></div></div>;
