...
try {
  const result = await svc.retryProvisioning(workspaceId, domainId);
  console.log(JSON.stringify({ ok: true, lifecycleState: result.lifecycleState, records: result.customerRecords?.map(r => ({ name: r.name, purpose: r.purpose })) }, null, 2));
} catch (error) {
