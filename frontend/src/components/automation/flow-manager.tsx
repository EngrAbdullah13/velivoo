...
export function FlowManager({ workspaceId }: {
    ...
    async function remove() { if (!deleteTarget) return; setDeleting(true); setDeleteError(''); try {
        await phase3Api(`/api/v1/workspaces/${workspaceId}/flows/${deleteTarget.id}`, { method: 'DELETE' });
        setDeleteTarget(null);
        setMenu(null);
        await load(undefined, true);
    }
    catch (error) {
        setDeleteError(error instanceof Error ? error.message : 'Unable to delete flow.');
  ...
  <ConfirmationDialog open={Boolean(deleteTarget)} title={`Delete ${deleteTarget?.name ?? 'flow'}?`} consequence="New entries and pending actions will stop. This flow will disappear from the workspace, but sent messages, versions, and run history remain for audit. Messages already submitted to the provider cannot be recalled." confirmLabel="Delete flow" pendingLabel="Deleting…" pending={deleting} error={deleteError} tone="danger" onCancel={() => { setDeleteTarget(null); setDeleteError(''); }} onConfirm={() => void remove()}/>
 </div>;
}
