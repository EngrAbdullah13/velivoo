...
const dnsRecordStatusLabel=(status:string)=>{const value=String(status??"pending").toLowerCase();if(value==="verified")return "Verified";if(value==="mismatch")return "Incorrect record";if(value==="nxdomain")return "Not found — add at your DNS provider";if(value==="verifying")return "Verifying";if(value==="timeout"||value==="servfail")return "Still propagating";return "Waiting for DNS";};
const staticCheckReady=(checks:any[],key:string)=>checks.find((check:any)=>check.key===key)?.status==="ready";
const staticRegistrarDnsReady=(domain:any,dmarcRequired:boolean)=>{const checks=domain.readinessChecks??[];const dmarcOk=!dmarcRequired||staticCheckReady(checks,"dmarc")||checks.find((check:any)=>check.key==="dmarc")?.status==="warning";return staticCheckReady(checks,"ownership")&&staticCheckReady(checks,"dkim")&&staticCheckReady(checks,"mail_from")&&dmarcOk;};
const staticMissingDnsLabels=(domain:any)=>{const records=domain.customerRecords??[];const missing:string[]=[];if(records.find((record:any)=>record.purpose==="ownership")?.status!=="verified")missing.push("ownership TXT (@)");if(records.find((record:any)=>record.purpose==="dkim_vm1")?.status!=="verified")missing.push("vm1 DKIM CNAME");if(records.find((record:any)=>record.purpose==="dkim_vm2")?.status!=="verified")missing.push("vm2 DKIM CNAME");if(records.find((record:any)=>record.purpose==="mail_from_mx")?.status!=="verified")missing.push("bounce MX");if(records.find((record:any)=>record.purpose==="mail_from_spf")?.status!=="verified")missing.push("bounce SPF TXT");return missing;};
const staticProgressMessage=(domain:any,dmarcRequired:boolean)=>{const missing=staticMissingDnsLabels(domain);if(missing.length)return `Add at your DNS provider, then Recheck DNS: ${missing.join(", ")}.`;const checks=domain.readinessChecks??[];for(const key of ["feedback","unsubscribe","sender"]){const check=checks.find((item:any)=>item.key===key);if(check&&check.status!=="ready"&&check.status!=="managed")return `${check.label} is not ready yet.`}const reason=Array.isArray(domain.readinessReasons)?domain.readinessReasons[0]:null;if(reason)return friendlyReason(String(reason));return String(domain.readinessStatus??"").toLowerCase()==="ready"?"Ready to send.":"Finish remaining platform checks.";};
const staticDnsRecordHeading=(count:number)=>`Add all ${count} DNS records for production sending`;
function StaticAwsVerifiedBanner({domain,dmarcRequired}:{domain:any;dmarcRequired:boolean}){const awsVerified=String(domain.verificationStatus??"").toLowerCase()==="success";const missing=staticMissingDnsLabels(domain);if(!awsVerified||missing.length===0)return null;return <div className="suppressions-note" role="status"><strong>AWS SES shows this domain as verified.</strong> Velivoo still checks public DNS at your registrar before sending is enabled. Missing or not visible yet: {missing.join(", ")}.</div>;}
function CustomerProgress({domain,isStatic=false,dmarcRequired=false}:{domain:any;isStatic?:boolean;dmarcRequired?:boolean}){const lifecycle=String(domain.lifecycleState??domain.status??"created").toLowerCase();if(isStatic){const checks=domain.readinessChecks??[],ready=String(domain.readinessStatus??"").toLowerCase()==="ready"||lifecycle==="ready",ownershipReady=staticCheckReady(checks,"ownership"),dkimDnsReady=staticCheckReady(checks,"dkim"),mailFromReady=staticCheckReady(checks,"mail_from"),dmarcReady=!dmarcRequired||staticCheckReady(checks,"dmarc")||checks.find((check:any)=>check.key==="dmarc")?.status==="warning",registrarReady=ownershipReady&&dkimDnsReady&&mailFromReady&&dmarcReady,identityReady=staticCheckReady(checks,"identity"),sesReady=identityReady&&String(domain.dkimStatus??"").toLowerCase()==="success",items=[{label:"DNS at your registrar",done:registrarReady},{label:"AWS email authentication",done:sesReady&&registrarReady},{label:"Ready to send",done:ready}];return <div className="onboarding-steps domain-progress">{items.map((item,index)=><div className="onboarding-step" key={item.label}><span>{item.done?"✓":index+1}</span><div><strong>{item.label}</strong>{!item.done&&index===items.findIndex(x=>!x.done)&&<p>{staticProgressMessage(domain,dmarcRequired)}</p>}</div></div>)}</div>}const rank:Record<string,number>={created:0,domain_created:0,infrastructure_provisioning:0,aws_resources_prepared:1,waiting_for_dns:1,awaiting_customer_dns:1,dns_verified:2,dns_delegated:2,delegation_verified:2,ownership_verified:3,ses_verifying:4,ses_verified:4,provider_provisioning:4,dkim_verifying:5,mail_from_configuring:5,authentication_verifying:5,mail_from_ready:6,tracking_ready:6,ready:7},current=rank[lifecycle]??0,items=[{label:"DNS records",done:current>=1},{label:"Delegation",done:current>=2},{label:"Ownership",done:current>=3},{label:"Email identity",done:current>=4},{label:"Ready",done:current>=7}];return <div className="onboarding-steps domain-progress">{items.map((item,index)=><div className="onboarding-step" key={item.label}><span>{item.done?"✓":index+1}</span><div><strong>{item.label}</strong>{!item.done&&index===items.findIndex(x=>!x.done)&&<p>{lifecycleCopy(lifecycle)}</p>}</div></div>)}</div>}
function StaticDnsGuidance({domain,dmarcRequired=false}:{domain:any;dmarcRequired?:boolean}){
  const root=domain.rootDomain??domain.domain;
  const dmarcReady=String(domain.readinessChecks?.find((check:any)=>check.key==="dmarc")?.status??"").toLowerCase()==="ready";
  const dmarcAdvisory=domain.dmarcAdvisory;
  return <section className="panel"><h2>Static DNS scope</h2><p className="panel-subtitle">Klaviyo-style static branded DNS. Every record below maps to real Velivoo routing, DKIM, return-path, or verification behavior.</p><dl><dt>Send routing CNAME</dt><dd><code>send.{root}</code> → Velivoo tracking/routing endpoint for click and open links.</dd><dt>DKIM vm1</dt><dd><code>vm1._domainkey.{root}</code> → active Velivoo Marketing selector.</dd><dt>DKIM vm2</dt><dd><code>vm2._domainkey.{root}</code> → standby selector for safe key rotation.</dd><dt>Return path MX + SPF</dt><dd><code>bounce.{root}</code> → two records with separate Priority and Mail server copy boxes (Hostinger-style).</dd><dt>Ownership TXT (@)</dt><dd>Root apex TXT <code>velivoo-site-verification=…</code> — verified independently from the email provider.</dd><dt>DMARC at <code>_dmarc.{root}</code></dt><dd>{dmarcReady?"Existing DMARC observed on recheck. Velivoo never creates or overwrites your record.":dmarcRequired?`Required for production sending. Add TXT ${dmarcAdvisory?.value??"v=DMARC1; p=none"} at _dmarc.`:"Optional but recommended: add your own DMARC TXT. Velivoo observes it on recheck and never overwrites an existing record."}</dd></dl></section>;
}
function readinessCheckDetail(check:any,domain:any,dmarcRequired:boolean){if(["ready","managed"].includes(check.status)){if(check.key==="identity")return "Verified in AWS SES";return check.status==="managed"?"Managed by Velivoo":"Ready";}if(check.key==="identity"&&String(domain.verificationStatus??"").toLowerCase()==="success")return "Verified in AWS — add missing registrar DNS records below";if(check.key==="ownership"||check.key==="send_routing"||check.key==="mail_from"){const record=domain.customerRecords?.find((row:any)=>({ownership:"ownership",send_routing:"send_routing",mail_from:"mail_from_mx"} as Record<string,string>)[check.key]===row.purpose);if(record?.status==="nxdomain")return "Not found in public DNS — add at your registrar";if(record?.status==="mismatch")return "Record found but value does not match";}if(check.key==="dmarc")return check.status==="ready"?"Observed at _dmarc — Velivoo never overwrites your record":dmarcRequired?"Required for production sending — add the DMARC TXT record shown above":"Add your own DMARC TXT at the root when ready";return "Waiting for public DNS or platform verification";}
function CustomerReadiness({domain,dmarcRequired=false}:{domain:any;dmarcRequired?:boolean}){const checks=domain.readinessChecks??[];if(!checks.length)return null;return <section className="panel"><h2>Sending readiness</h2><p className="panel-subtitle">AWS verification alone is not enough for static DNS. Velivoo also checks the records you publish at your registrar.</p><div className="health-list">{checks.map((check:any)=><div className="health-row" key={check.key}><span className="health-icon">{["ready","managed"].includes(check.status)?"✓":"!"}</span><div><strong>{check.label}</strong><small>{readinessCheckDetail(check,domain,dmarcRequired)}</small></div><span className={`pill pill-${stateTone(check.status)}`}>{check.status==="managed"?"Platform managed":title(check.status)}</span></div>)}</div></section>}
function domainVerified(domain:any){return domain.status==="verified"||domain.authenticationStatus==="verified"||String(domain.lifecycleState??"").toUpperCase()==="READY";}
function SendingPurposePanel({domain,workspaceId,busy,action,canManage,compact=false}:any){
  const [purpose,setPurpose]=useState<"marketing"|"transactional"|"">(domain.sendingPurpose??"");
  useEffect(()=>setPurpose(domain.sendingPurpose??""),[domain.id,domain.sendingPurpose]);
  if(!domainVerified(domain))return null;
  const save=()=>purpose&&action(`purpose-${domain.id}`,`/api/v1/workspaces/${workspaceId}/sender-domains/${domain.id}/purpose`,{purpose},"PATCH");
  return <section className={`sending-purpose-panel${compact?" compact":""}`} aria-labelledby={`sending-purpose-${domain.id}`}>
    <div className="sending-purpose-heading"><div><span className="eyebrow">{domain.sendingPurpose?"Email use case":"Next step"}</span><h3 id={`sending-purpose-${domain.id}`}>What will you send?</h3><p>Choose the intended use of this domain in Velivoo. This does not change your AWS SES account classification or control whether Gmail places a message in Primary or Promotions.</p></div>{domain.sendingPurpose&&<span className="pill pill-success">Use case saved</span>}</div>
    <div className="sending-purpose-options" role="radiogroup" aria-label="Email use case">
      <button type="button" role="radio" aria-checked={purpose==="marketing"} className={`sending-purpose-option${purpose==="marketing"?" selected":""}`} onClick={()=>setPurpose("marketing")} disabled={!canManage}><span className="sending-purpose-icon">✦</span><span><strong>Marketing</strong><small>Newsletters, promotions, campaigns, and nurture automations. Consent, unsubscribe, and frequency protections apply.</small></span></button>
      <button type="button" role="radio" aria-checked={purpose==="transactional"} className={`sending-purpose-option${purpose==="transactional"?" selected":""}`} onClick={()=>setPurpose("transactional")} disabled={!canManage}><span className="sending-purpose-icon">↗</span><span><strong>Transactional</strong><small>Receipts, security alerts, account updates, and other necessary one-to-one messages. Not for promotional list sends.</small></span></button>
    </div>
    {purpose==="transactional"&&<p className="suppressions-note" role="status">Transactional means one-to-one operational messages such as receipts or password resets. Campaigns, newsletters, promotional outreach, and marketing flows remain marketing messages even when this domain is labelled transactional here. AWS SES account mail type is configured separately for the whole account and region.</p>}
    {canManage&&<div className="button-row"><button type="button" className="button-primary" onClick={save} disabled={!purpose||purpose===domain.sendingPurpose||busy===`purpose-${domain.id}`}>{busy===`purpose-${domain.id}`?"Saving…":domain.sendingPurpose?"Save use case":"Continue"}</button>{!domain.sendingPurpose&&<small>Required before creating a sender identity.</small>}</div>}
  </section>;
}
...
function Domains({ data, workspaceId, busy, action, error }: any) {
  ...
  const currentDomain=data.domains[0]??null,brandedAvailable=Boolean(data.capabilities.brandedDomainSetupAvailable),staticAvailable=Boolean(data.capabilities.staticBrandedSetupAvailable),dmarcRequired=Boolean(data.capabilities.dmarcRequired);
  ...
  return <div className="deliverability-stack">
    ...
    {data.domains.map((item: any) => {
      ...
      return <section key={item.id} className="panel domain-card">
        ...
        <SendingPurposePanel domain={item} workspaceId={workspaceId} busy={busy} action={action} canManage={data.capabilities.canManageDomains} compact/>
        {!managed?<p className="suppressions-note">This existing sender remains available in legacy compatibility mode. It is not silently migrated or removed.</p>:<div className="dns-setup">
          {isStatic&&<StaticAwsVerifiedBanner domain={item} dmarcRequired={dmarcRequired}/>}
          <CustomerProgress domain={item} isStatic={isStatic} dmarcRequired={dmarcRequired}/>
          <div className="dns-instructions"><span className="setup-step">1</span><div><h3>{isStatic?staticDnsRecordHeading(item.productionDnsRecordCount??(dmarcRequired?6:5)):isV3?"Add five DNS records":"Add all seven DNS records"}</h3><p>{isStatic?<>Every domain needs exactly {item.productionDnsRecordCount??(dmarcRequired?6:5)} registrar records for safe production sending. Verified records stay visible with a green status.</>:isV3?<>Add four NS records that delegate {item.infraDomain??`send.${item.rootDomain??item.domain}`} to Velivoo, plus the ownership TXT record. After delegation, Velivoo manages SES Easy DKIM, MAIL FROM, and tracking inside that zone.</>:<>Add four NS records for the delegated infrastructure subdomain and three DKIM CNAME records at the DNS provider for {item.rootDomain??item.domain}. Velivoo manages MAIL FROM and tracking records after delegation.</>}</p></div></div>
  ...
  return <div className="deliverability-stack">
    ...
    <section className="panel domain-card">
      ...
      {managed&&<div className="dns-setup">
        {isStatic&&<StaticAwsVerifiedBanner domain={domain} dmarcRequired={dmarcRequired}/>}
        <CustomerProgress domain={domain} isStatic={isStatic} dmarcRequired={dmarcRequired}/>
        <div className="dns-instructions"><span className="setup-step">1</span><div><h3>{isStatic?staticDnsRecordHeading(domain.productionDnsRecordCount??(dmarcRequired?6:5)):isV3?"Publish five customer DNS records":"Publish the seven customer DNS records"}</h3><p>{isStatic?`Add every record exactly as shown — ${domain.productionDnsRecordCount??(dmarcRequired?6:5)} records total for safe production email. Already-verified records remain listed below.` :isV3?`Add four NS records for ${domain.infraDomain} and the Velivoo ownership TXT record. Velivoo manages authentication, mail-from, and tracking records inside the delegated zone.`: `Add four NS records for ${domain.infraDomain} and all three DKIM CNAME records for the root identity. Mail-from and tracking records remain Velivoo-managed.`}</p></div></div>
    ...
    <SendingPurposePanel domain={domain} workspaceId={workspaceId} busy={busy} action={action} canManage={data.capabilities.canManageDomains}/>
    {isStatic&&<StaticDnsGuidance domain={domain} dmarcRequired={dmarcRequired}/>} 
    <CustomerReadiness domain={domain} dmarcRequired={dmarcRequired}/>
    ...
    <div>
      <ul className={`dns-records dns-records-${kind}`}>
        {items.map((record, index) => {
          ...
          const statusLabel = dnsRecordStatusLabel(status);
          if (presentation) {
            return (
              <li key={key}>
