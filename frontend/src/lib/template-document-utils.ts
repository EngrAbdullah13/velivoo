...
export function buildImportedHtmlDocument(html: string, previous?: StructuredEmailDocument): StructuredEmailDocument {
  const footer = previous?.blocks.find(block => block.type === "compliance_footer");
  return {
    schemaVersion: 1,
    blocks: [
